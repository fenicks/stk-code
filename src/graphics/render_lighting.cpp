...
void IrrDriver::renderLights(unsigned pointlightcount)
{
    ...
    if (UserConfigParams::m_gi)
    {
        ...
        else
        {
            ...
            FullScreenShader::RadianceHintsConstructionShader::getInstance()->SetTextureUnits({ m_rtts->getRSM().getRTT()[0], m_rtts->getRSM().getRTT()[1], m_rtts->getRSM().getDepthTexture() });
